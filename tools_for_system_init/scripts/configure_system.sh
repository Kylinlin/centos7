...
function Security_Setting {
        ...
        #Lock account for 5 minutes after input password incorrectly in 3 times
        sed -i 's#auth        required      pam_env.so#auth        required      pam_env.so\nauth       required       pam_tally.so  onerr=fail deny=3 unlock_time=300\nauth           required     /lib/security/$ISA/pam_tally.so onerr=fail deny=3 unlock_time=300#' /etc/pam.d/system-auth

        #logout after 10 minutes without any operations
        ...
        #Active syncookie to avoid ddos attack
        ...
        #Limit the privilege of import command
        chmod 700 /bin/ping
        ...
        #Secure the command history file
        chattr +a /root/.bash_history
        chattr +i /root/.bash_history

        #Write md5 to import command
        cat > list << "EOF" &&
        /bin/ping
        /usr/bin/finger
        /usr/bin/who
